...
//rename & upload file
	...
	echo $uploadexcel['name'];
		...
		foreach ($namedDataArray as $result) {
			$cp_pertitle=trim(iconv('utf-8','tis-620',$result['คำนำหน้าชื่อ']));
			$cp_pername=trim(iconv('utf-8','tis-620',$result['ชื่อ']));
			$cp_persurname=trim(iconv('utf-8','tis-620',$result['นามสกุล']));
			$cp_mumail=trim(iconv('utf-8','tis-620',$result['MUmail']));
			$cp_dpname=trim(iconv('utf-8','tis-620',$result['ส่วนงาน']));
