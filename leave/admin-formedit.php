	...
	<div class="row">
    	<div class="col-sm-12">
        	...
        	<div class="page-header-05">
                <div class="text-title"><a href="<?php echo $livesite;?>leave/admin-allform.php"><i class="fa fa-arrow-left fa-fw"></i> แบบฟอร์มใบลาไปเพิ่มพูนความรู้และประสบการณ์ (สำหรับผู้รับผิดชอบข้อมูล)</a></div>
            </div>
            <form method="post" id="formDefault" action="admin-formedit-action.php">
            	<!--ข้อมูลผู้ขออนุมัติ-->
                <?php
                ...
                <div class="row">
                	<div class="col-sm-6 col-sm-offset-6">
                    	<div class="form-group form-group-sm">
                            ...
                            <input name="dev_orddate" type="text" class="form-control" id="dev_orddate" required value="<?php echo $rPersonel['dev_orddate'];?>">
                        </div>
                    </div><!--col-->
                </div><!--row-->
                    	...
                    	<div class="form-group form-group-sm">
                            ...
                            <select name="per_name" class="form-control select select-inverse select-sm" data-toggle="select" required>
								...
								$qPersonel02=mysql_query("select * from $db_eform.personel_muerp as t1
									left join $db_eform.personel_status as t2 on (t1.per_status=t2.ps_id)
								...
								while($rPersonel02=mysql_fetch_assoc($qPersonel02)){
									if($rPersonel['per_id']==$rPersonel02['per_id']){
										echo '<option value="'.$rPersonel02['per_id'].'" selected>'.$rPersonel02['per_fnamet'].' '.$rPersonel02['per_lnamet'].'</option>';
									}else{
										echo '<option value="'.$rPersonel02['per_id'].'">'.$rPersonel02['per_fnamet'].' '.$rPersonel02['per_lnamet'].'</option>';
									}
								}
								?>
                            </select>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="form-group form-group-sm">
                        	<label class="control-label">ความเห็นต้นสังกัด:</label>
                            <div class="row">
                            <?php
							foreach($cf_devleave_status as $kStatus=>$vStatus){
								if($rPersonel['dev_formstatus']==$kStatus){
									echo '<div class="col-sm-4"><label class="radio"><input name="dev_formstatus" type="radio" value="'.$kStatus.'" data-toggle="radio" checked required> '.$vStatus['name'].'</label></div>';
								}else{
									echo '<div class="col-sm-4"><label class="radio"><input name="dev_formstatus" type="radio" value="'.$kStatus.'" data-toggle="radio" required> '.$vStatus['name'].'</label></div>';
								}
							}
							?>
                            </div><!--row-->
                            <input name="dev_formstatus_comment" type="text" class="form-control" placeholder="อื่นๆ ระบุ" value="<?php echo $rPersonel['dev_formstatus_comment'];?>">
                        </div>
                        ...
                        <div class="form-group form-group-sm">
                        	<div class="row">
                            	<div class="col-sm-6">
                                    ...
                                    <input type="text" name="dev_bossname" class="form-control" maxlength="100" required value="<?php print $rPersonel['dev_bossname'];?>">
                                </div><!--col-->
                                <div class="col-sm-6">
                                	<label class="control-label">ตำแหน่งผู้ลงนามอนุมัติ:</label>
                                    <input type="text" name="dev_bosspos" class="form-control" maxlength="100" required value="<?php print $rPersonel['dev_bosspos'];?>">
                                </div><!--col-->
                            </div><!--row-->
                        </div>
...
$(document).ready(function(e) {
	...
	$('#per_contract1').datepicker()
        ...
        .on('change show', function(e) {
            // Validate the date when user change it
            $('#formDefault').data('bootstrapValidator').revalidateField('dev_orddate');
            // You also can call it as following:
            //$('#formFilter').bootstrapValidator('revalidateField', 'startDate');
        });
