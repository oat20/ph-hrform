...
function smtpmail( $email , $subject , $body, $from='noreply@'.$_SERVER['HTTP_HOST'], $fromname=strtoupper($_SERVER['HTTP_HOST']))
{
    $mail = new PHPMailer();
    $mail->IsSMTP();         
...
function smtpmail2( $email , $subject , $body, $from='noreply@'.$_SERVER['HTTP_HOST'], $fromname=strtoupper($_SERVER['HTTP_HOST']) )
{
...
function smtpGmail( $email , $subject , $body )
{
    ...
    $mail->Username = "phmahidol1948@outlook.com";   //  account e-mail ??????????????????
    $mail->Password = "P2491220907h";  //  ???????? e-mail ??????????????????

    $mail->From     = "phmahidol1948@outlook.com";  //  account e-mail ???????????????????????
