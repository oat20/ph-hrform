...
<div class="container-fluid">
	...
	?>
    <form action="2_editform2_action.php" method="post" id="formDefault" enctype="multipart/form-data">
    	<div class="panel panel-default">
        	<div class="panel-heading clearfix">
            	<h3 class="panel-title pull-left"><a href="2_allform.php"><i class="fa fa-arrow-left fa-fw"></i> แก้ไขแบบฟอร์มอนุมัติปฏบัติงานบริการวิชาการ</a></h3>
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="table table-striped">
        ...
        <tr >
                ...
                <div class="form-group">
					...
					$typ=mysql_query("select * from $db_eform.develop_main_type
                        ...
                        while($ob=mysql_fetch_array($typ)){
                            if($rDevelop['dev_type']==$ob['dvt_id']){
                            	print '<label class="radio"><input type="radio" data-toggle="radio" name="dvt_id" value="'.$ob[dvt_id].'" checked>'.$ob['dvt_name'].'</label>';
							}else{
								print '<label class="radio"><input type="radio" data-toggle="radio" name="dvt_id" value="'.$ob[dvt_id].'">'.$ob['dvt_name'].'</label>';
							}
                        }
					?>
                    </div>
                    <div class="form-group">
                    	<div id="devTypeother">
                            ...
                            <input type="text" name="dev_typeother" class="form-control input-sm" <?php echo $disabled;?> placeholder="อื่นๆ ระบุ" value="<?php echo $rDevelop['dev_typeother'];?>">
                        </div>
                    </div>         
                    </td>
            </tr>
             ...
             <tr>
            	<td colspan="2">
                	<div class="form-group">
                        ...
                        <select name="le_id" class="form-control select select-inverse select-sm" data-toggle="select">
                        	<?php
							$sql=mysql_query("select * from $db_eform.develop_level
								where le_use='yes'
								order by le_id asc");
							while($ob=mysql_fetch_assoc($sql)){
								if($rDevelop['le_id']==$ob['le_id']){
									echo '<option value="'.$ob['le_id'].'" selected>&raquo; '.$ob['le_title'].'</option>';
								}else{
									echo '<option value="'.$ob['le_id'].'">&raquo; '.$ob['le_title'].'</option>';
								}
							}
							?>
                        </select>
                	...
                	<div class="form-group">
                        ...
                        <select name="dev_approve" class="form-control select select-danger" data-toggle="select">
							...
							foreach($cf_approve as $k=>$v){
								if($rDevelop['dev_approve']==$k){
									echo '<option value="'.$k.'" selected>&raquo; '.$v['name'].'</option>';
								}else{
            ...
            <div class="panel-footer">
            	...
            	<input class="btn btn-inverse btn-block" type="submit" value="บันทึกแบบฟอร์ม">
            </div><!--footer-->
        </div><!--panel-->
    </form>
...
$(document).ready(function(e) {
    ...
    $('input[name="dvt_id"]').on('change', function() {
