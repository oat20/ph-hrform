        ...
        <div class="col-lg-12">
        	<div class="panel panel-info">
            	<div class="panel-heading">
                	<h3 class="panel-title">จำแนกตามส่วนงาน</h3>
                </div>
                <div class="panel-body">
                        	...
                        	<div class="row">
                                ...
                                <div class="col-lg-4">
                                	<div class="form-group">
                                        ...
                                        <select name="dp_id" class="form-control select select-primary select-sm" data-toggle="select">
                                            <option value="">ทั้งหมด</option>
                                            <?php
                                            $sql=mysql_query("select * from phper2.department_type
													...
													tb_orgnew.dp_id asc");
                                            while($ob=mysql_fetch_assoc($sql)){
												if($ob['dp_id']==$_POST['dp_id']){
                                                	echo '<option value="'.$ob['dp_id'].'" selected>'.$ob['typ_name'].' &raquo; '.$ob['dp_name'].'</option>';
												}else{
													echo '<option value="'.$ob['dp_id'].'">'.$ob['typ_name'].' &raquo; '.$ob['dp_name'].'</option>';
												}
                                            }
                                            ?>
                                        </select>
                    ...
                    <!--filter-->
                	<p class="text-right"><a href="" target="_blank"><i class="fa fa-file-excel-o"></i> Download Excel</a></p>
								...
								if(empty($_POST['keyDevstdate']) and empty($_POST['keyDevenddate'])){
									...
									if($_POST['dp_id'] == ''){
										...
										$sql=mysql_query("select * from phper2.develop
												...
												where (develop.dev_stdate between '$_POST[keyDevstdate]' and '$_POST[keyDevenddate]'
												or develop.dev_enddate between '$_POST[keyDevstdate]' and '$_POST[keyDevenddate]')
												and personel.per_dept='$_POST[dp_id]'
												order by develop.dev_stdate desc,
												develop.dev_enddate desc");
									}
