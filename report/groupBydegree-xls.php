...
require_once "../lib/writeexcel/class.writeexcel_workbook.inc.php";
			...
			$xlsNumberformat=& $workbook->addformat(array(num_format => '#,##'));
			//$xlsNumberformat=& $workbook->addformat(array(num_format => '#,##0.00'));
            ## End of Set Format ##
            ...
            $worksheet->write(A1,iconv("utf-8","tis-620","รายงานจำแนกตามระดับการศึกษาของบุคลากร"), $xlscelldesc_header);
			...
			$worksheet->write_blank(V1,$xlscelldesc_header);
			$worksheet->write_blank(W1,$xlscelldesc_header);
			$worksheet->write_blank(X1,$xlscelldesc_header);
			$worksheet->write_blank(Y1,$xlscelldesc_header);
            ...
            $worksheet->write(A2,iconv("utf-8","tis-620","ลำดับ"), $xlscelldesc_header1);//row&column, text, style
			 $worksheet->write(B2, 'Ref No.', $xlscelldesc_header1); 
			  ...
			  $worksheet->write(D2,iconv("utf-8","tis-620","การศึกษา"), $xlscelldesc_header1);  
            $worksheet->write(E2,iconv("utf-8","tis-620","กลุ่ม"), $xlscelldesc_header1);
            $worksheet->write(F2,iconv("utf-8","tis-620","ประเภท"), $xlscelldesc_header1);
            $worksheet->write(G2,iconv("utf-8","tis-620","ตำแหน่ง"), $xlscelldesc_header1);
			$worksheet->write(H2,iconv("utf-8","tis-620","ตวช."), $xlscelldesc_header1);
			$worksheet->write(I2,iconv("utf-8","tis-620","ส่วนงาน"), $xlscelldesc_header1);
			$worksheet->write(J2,iconv("utf-8","tis-620","หลักสูตร"), $xlscelldesc_header1);
			$worksheet->write(K2,iconv("utf-8","tis-620","สถานที่"), $xlscelldesc_header1);
			$worksheet->write(L2,iconv("utf-8","tis-620","เริ่ม"), $xlscelldesc_header1);
			$worksheet->write(M2,iconv("utf-8","tis-620","ถึง"), $xlscelldesc_header1);
			$worksheet->write(N2,iconv("utf-8","tis-620","พัฒนาบุคลากร"), $xlscelldesc_header1);
			$worksheet->write(O2,iconv("utf-8","tis-620","บริการวิชาการ"), $xlscelldesc_header1);
			$worksheet->write(P2,iconv("utf-8","tis-620","วัตถุประสงค์"), $xlscelldesc_header1);
			$worksheet->write(Q2,iconv("utf-8","tis-620","ชม."), $xlscelldesc_header1);
			$worksheet->write(R2,iconv("utf-8","tis-620","กิจกรรม"), $xlscelldesc_header1);
			$worksheet->write(S2,iconv("utf-8","tis-620","ยุทธศาสตร์ ม."), $xlscelldesc_header1);
			$worksheet->write(T2,iconv("utf-8","tis-620","ยุทธสาสตร์ คณะฯ"), $xlscelldesc_header1);
			$worksheet->write(U2,iconv("utf-8","tis-620","งบประมาณ"), $xlscelldesc_header1);
			$worksheet->write(V2,iconv("utf-8","tis-620","หลักการ"), $xlscelldesc_header1);
			$worksheet->write(W2,iconv("utf-8","tis-620","อนุมัติ"), $xlscelldesc_header1);
			$worksheet->write(X2,iconv("utf-8","tis-620","จ่ายจริง"), $xlscelldesc_header1);
			$worksheet->write(Y2,iconv("utf-8","tis-620","หน่วยงานผู้จัด"), $xlscelldesc_header1);
            $xlsRow = 3;
			...
			$sql_1= "select * from $db_eform.develop
						...
						inner join $db_eform.education on (personel_muerp.per_id = education.ed_perid)
				...
				//งบประมาณ
                        ...
                        $worksheet->write("A$xlsRow", "$i", $xlsCellDesc);
						 $worksheet->write("B$xlsRow", $ob['dev_id'], $xlsCellDesc);
						  ...
						  $worksheet->write("D$xlsRow", iconv("utf-8","tis-620",$ob['dg_name']), $xlsCellDesc);
                        ...
                        $worksheet->write("G$xlsRow", iconv('utf-8','tis-620',$ob['job_id']), $xlsCellDesc);
						$worksheet->write("H$xlsRow", iconv("utf-8","tis-620",$ob['ja_name']),  $xlsCellDesc);
						$worksheet->write("I$xlsRow", iconv("utf-8","tis-620",$ob['dp_name']),  $xlsCellDesc);
						$worksheet->write("J$xlsRow", iconv("utf-8","tis-620",$ob['dev_onus']),  $xlsCellDesc);
						$worksheet->write("K$xlsRow", iconv("utf-8","tis-620",$ob['dev_place'].' ('.$ob['lt_title'].', '.$cf_local[$ob['dev_local']]['name'].')'),  $xlsCellDesc);
						$worksheet->write("L$xlsRow", iconv("utf-8","tis-620",dateFormat($ob['dev_stdate'])),  $xlsCellDesc);
						$worksheet->write("M$xlsRow", iconv("utf-8","tis-620",dateFormat($ob['dev_enddate'])),  $xlsCellDesc);
						$worksheet->write("N$xlsRow", iconv("utf-8","tis-620",$devMaintype01),  $xlsCellDesc_02);
						$worksheet->write("O$xlsRow", iconv("utf-8","tis-620",$devMaintype02),  $xlsCellDesc_02);
						$worksheet->write("P$xlsRow", iconv('utf-8','tis-620',substr($dvt_name,'0','-2')),  $xlsCellDesc);
						$worksheet->write("Q$xlsRow", iconv("utf-8","tis-620",$ob['dev_training_hour']),  $xlsCellDesc);
						$worksheet->write("R$xlsRow", iconv("utf-8","tis-620",$ob['activity']),  $xlsCellDesc);
						$worksheet->write("S$xlsRow", iconv("utf-8","tis-620",$ob['nameth']),  $xlsCellDesc);
						$worksheet->write("T$xlsRow", iconv("utf-8","tis-620",$ob['sf_name']),  $xlsCellDesc);
						$worksheet->write("U$xlsRow", iconv('utf-8','tis-620',substr($budget,'0','-2')),  $xlsCellDesc);
						$worksheet->write("V$xlsRow", $ob['budget_pay01'],  $xlsNumberformat);
						$worksheet->write("W$xlsRow", $ob['budget_pay02'],  $xlsNumberformat);
						$worksheet->write("X$xlsRow", $ob['budget_pay03'],  $xlsNumberformat);
						$worksheet->write("Y$xlsRow", iconv("utf-8","tis-620",$ob['dev_org']),  $xlsCellDesc);
                $xlsRow++;
                }				
				//จำนวนรวม
				/*$worksheet->set_row($xlsRow, 22);
