...
require_once "../lib/writeexcel/class.writeexcel_workbook.inc.php";
            ...
            $worksheet->write(A1,iconv("utf-8","tis-620","รายงานจำแนกตามวัตถุประสงค์"), $xlscelldesc_header);
            ...
            /*$worksheet->write(A2,iconv("utf-8","tis-620","#"), $xlscelldesc_header1);//row&column, text, style
			 $worksheet->write(B2, 'Form No.', $xlscelldesc_header1); 
			  $worksheet->write(C2,iconv("utf-8","tis-620","ผู้บันทึกข้อมูล"), $xlscelldesc_header1);  
			...
			$worksheet->write(D2,iconv("utf-8","tis-620","หลักสูตร/โครงการ"), $xlscelldesc_header1);
			$worksheet->write(E2,iconv("utf-8","tis-620","สถานที่"), $xlscelldesc_header1);
			$worksheet->write(F2,iconv("utf-8","tis-620","วันที่"), $xlscelldesc_header1);
			$worksheet->write(G2,iconv("utf-8","tis-620","เวลา"), $xlscelldesc_header1);
			$worksheet->write(H2,iconv("utf-8","tis-620","พัฒนาบุคลากร"), $xlscelldesc_header1);
			$worksheet->write(I2,iconv("utf-8","tis-620","บริการวิชาการ"), $xlscelldesc_header1);
			$worksheet->write(J2,iconv("utf-8","tis-620","วัตถุประสงค์"), $xlscelldesc_header1);
			$worksheet->write(K2,iconv("utf-8","tis-620","ชม."), $xlscelldesc_header1);
			$worksheet->write(L2,iconv("utf-8","tis-620","กิจกรรม"), $xlscelldesc_header1);
			$worksheet->write(M2,iconv("utf-8","tis-620","ยุทธศาสตร์ ม."), $xlscelldesc_header1);
			$worksheet->write(N2,iconv("utf-8","tis-620","ยุทธสาสตร์ คณะฯ"), $xlscelldesc_header1);
			$worksheet->write(O2,iconv("utf-8","tis-620","งบประมาณ"), $xlscelldesc_header1);
			$worksheet->write(P2,iconv("utf-8","tis-620","หน่วยงานผู้จัด"), $xlscelldesc_header1);
			$worksheet->write(Q2,iconv("utf-8","tis-620","จำนวผู้เข้าร่วม"), $xlscelldesc_header1);
			 $worksheet->write(R2,iconv("utf-8","tis-620","ผู้บันทึกข้อมูล"), $xlscelldesc_header1);
            $xlsRow = 3;
			...
			/*$sql_1= "select * from $db_eform.develop
						...
						inner join $db_eform.develop_form_objective on (develop.dev_id = develop_form_objective.dev_id)
						...
						and develop_form_objective.dvt_id = '$_GET[dvt_id]'
			...
			$sql_1= "select * from $db_eform.develop
						...
						or develop.dev_enddate between '$_GET[dev_stdate]' and '$_GET[dev_enddate]')
						and develop.dev_maintype='$_GET[dm_id]'
						and develop.dev_type = '$_GET[dvt_id]'
						and develop.dev_cancel = 'no'
						and develop.dev_remove = 'no'
						order by develop.dev_stdate desc,
						develop.dev_enddate desc";
            ...
            # ตรงนี้คือดึงข้อมูลจาก mysql มาใส่ใน Cell
				...
				while($ob = mysql_fetch_array($rs)){
					...
					//จำนวนผู้เข้าร่วม
					$sql02=mysql_query("select count(cp_id) as countCpid from $db_eform.develop_course_personel where dev_id = '$ob[dev_id]'");
					...
					//งบประมาณ
						...
						$worksheet->write("C$xlsRow", iconv("utf-8","tis-620",$ob['dp_name']),  $xlsCellDesc);
						$worksheet->write("D$xlsRow", iconv("utf-8","tis-620",$ob['dev_onus']),  $xlsCellDesc);
						$worksheet->write("E$xlsRow", iconv("utf-8","tis-620",$ob['dev_place'].' ('.$ob['lt_title'].')'),  $xlsCellDesc);
						$worksheet->write("F$xlsRow", iconv("utf-8","tis-620",dateFormat($ob['dev_stdate']).' - '.dateFormat($ob['dev_enddate'])),  $xlsCellDesc);
						$worksheet->write("G$xlsRow", iconv("utf-8","tis-620", $ob['dev_timebegin'].' - '.$ob['dev_timeend']),  $xlsCellDesc);
						$worksheet->write("H$xlsRow", iconv("utf-8","tis-620",$devMaintype01),  $xlsCellDesc_02);
						$worksheet->write("I$xlsRow", iconv("utf-8","tis-620",$devMaintype02),  $xlsCellDesc_02);
						$worksheet->write("J$xlsRow", iconv('utf-8','tis-620',$ob['dvt_name']),  $xlsCellDesc);
						$worksheet->write("K$xlsRow", iconv("utf-8","tis-620",Showtraininghour($ob['dev_training_hour'])),  $xlsCellDesc);
						$worksheet->write("L$xlsRow", iconv("utf-8","tis-620",$ob['activity']),  $xlsCellDesc);
						$worksheet->write("M$xlsRow", iconv("utf-8","tis-620",$ob['nameth']),  $xlsCellDesc);
						$worksheet->write("N$xlsRow", iconv("utf-8","tis-620",$ob['sf_name']),  $xlsCellDesc);
						$worksheet->write("O$xlsRow", iconv('utf-8','tis-620',$budget),  $xlsCellDesc);
						$worksheet->write("P$xlsRow", iconv("utf-8","tis-620",$ob['dev_org']),  $xlsCellDesc);
						$worksheet->write("Q$xlsRow", number_format($ob02['countCpid']),  $xlsCellDesc);
					  $worksheet->write("R$xlsRow", iconv("utf-8","tis-620",$ob['per_pname'].' '.$ob['per_fnamet'].' '.$ob['per_lnamet']), $xlsCellDesc);
            ...
            $workbook->close();
                ...
                header("Content-Disposition: attachment; filename=".basename($titlebar['shorttitle'].".xls").";");
