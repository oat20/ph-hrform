...
require_once "../lib/writeexcel/class.writeexcel_workbook.inc.php";
            ...
            $worksheet->write(A2,iconv("utf-8","tis-620","ลำดับ"), $xlscelldesc_header1);//row&column, text, style
			 $worksheet->write(B2, 'Form ID', $xlscelldesc_header1); 
			...
			$worksheet->write(U2,iconv("utf-8","tis-620","หลักการ"), $xlscelldesc_header1);
			$worksheet->write(V2,iconv("utf-8","tis-620","อนุมัติ"), $xlscelldesc_header1);
			$worksheet->write(W2,iconv("utf-8","tis-620","จ่ายจริง"), $xlscelldesc_header1);
			$worksheet->write(X2,iconv("utf-8","tis-620","หน่วยงานผู้จัด"), $xlscelldesc_header1);
			$worksheet->write(Y2,iconv("utf-8","tis-620","ดาวน์โหลดแบบฟอร์ม"), $xlscelldesc_header1);
            $xlsRow = 3;
			
				$sql_1= "select * 
					from $db_eform.develop
							left join $db_eform.develop_course_personel on (develop.dev_id = develop_course_personel.dev_id)
							left join $db_eform.personel_muerp on (develop_course_personel.per_id = personel_muerp.per_id)
							left join $db_eform.personel_group on (personel_muerp.per_group = personel_group.gr_id)
							left join $db_eform.personel_type on (personel_muerp.per_type = personel_type.pert_id)
							left join $db_eform.job on (personel_muerp.job_id=job.job_id)
							left join $db_eform.job_academic on (personel_muerp.ja_id = job_academic.ja_id)				
							left join $db_eform.tb_orgnew on (personel_muerp.per_dept = tb_orgnew.dp_id)
            ...
            # ตรงนี้คือดึงข้อมูลจาก mysql มาใส่ใน Cell
				...
				//งบประมาณ
                        ...
                        $worksheet->write("F$xlsRow", iconv('utf-8','tis-620',$ob['job_name']), $xlsCellDesc);
						...
						$worksheet->write("T$xlsRow", iconv('utf-8','tis-620',substr($budget,'0','-2')),  $xlsCellDesc);
						$worksheet->write("U$xlsRow", number_format($ob['budget_pay01']),  $xlsCellDesc);
						$worksheet->write("V$xlsRow", number_format($ob['budget_pay02']),  $xlsCellDesc);
						$worksheet->write("W$xlsRow", number_format($ob['budget_pay03']),  $xlsCellDesc);
						$worksheet->write("X$xlsRow", iconv("utf-8","tis-620",$ob['dev_org']),  $xlsCellDesc);
						$worksheet->write("Y$xlsRow", $livesite.'form/print-form01-pdf.php?getTrackid='.$ob['dev_trackid']);
                $xlsRow++;
                }				
				//จำนวนรวม
				/*$worksheet->set_row($xlsRow, 22);
            ...
            $workbook->close();
                ...
                header("Content-Disposition: attachment; filename=".basename("PH_".$_COOKIE['PHPSESSID'].".xls").";");
