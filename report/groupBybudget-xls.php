...
require_once "../lib/writeexcel/class.writeexcel_workbook.inc.php";
            ...
            $worksheet->write(A1,iconv("utf-8","tis-620","รายงานจำแนกตามประเภทงบประมาณ"), $xlscelldesc_header);
			...
			$sql_1= "select * from $db_eform.develop
						...
						inner join $db_eform.personel_group on (personel.per_group = personel_group.gr_id)
						inner join $db_eform.personel_type on (personel.per_type = personel_type.pert_id)
						inner join $db_eform.job_academic on (personel.ja_id = job_academic.ja_id)
						inner join $db_eform.tb_orgnew on (personel.per_dept = tb_orgnew.dp_id)
						...
						and develop_form_budget.bt_id = '$_GET[bt_id]'
            ...
            # ตรงนี้คือดึงข้อมูลจาก mysql มาใส่ใน Cell
				...
				//วัตถุประสงค์
						...
						$worksheet->write("T$xlsRow", iconv('utf-8','tis-620',$ob['bt_name']),  $xlsCellDesc);
						$worksheet->write("U$xlsRow", number_format($ob['dev_pay01']),  $xlsCellDesc);
						$worksheet->write("V$xlsRow", number_format($ob['dev_pay02']),  $xlsCellDesc);
						$worksheet->write("W$xlsRow", number_format($ob['dev_pay03']),  $xlsCellDesc);
