...
require_once "../lib/writeexcel/class.writeexcel_workbook.inc.php";
			...
			$worksheet->write(A2,iconv("utf-8","tis-620","#"), $xlscelldesc_header1);
			$worksheet->write(B2,iconv("utf-8","tis-620","Status"), $xlscelldesc_header1);//row&column, text, style
			 $worksheet->write(C2, 'Form No.', $xlscelldesc_header1); 		   
			$worksheet->write(D2,iconv("utf-8","tis-620","ส่วนงาน"), $xlscelldesc_header1);
			$worksheet->write(E2, '', $xlscelldesc_header1);
			$worksheet->write(F2,iconv("utf-8","tis-620","หลักสูตร/โครงการ"), $xlscelldesc_header1);
			$worksheet->write(G2,iconv("utf-8","tis-620","สถานที่"), $xlscelldesc_header1);
			$worksheet->write(H2,iconv("utf-8","tis-620","วันที่"), $xlscelldesc_header1);
			$worksheet->write(I2,iconv("utf-8","tis-620","เวลา"), $xlscelldesc_header1);
			$worksheet->write(J2,iconv("utf-8","tis-620","พัฒนาบุคลากร"), $xlscelldesc_header1);
			$worksheet->write(K2,iconv("utf-8","tis-620","บริการวิชาการ"), $xlscelldesc_header1);
			$worksheet->write(L2,iconv("utf-8","tis-620","วัตถุประสงค์"), $xlscelldesc_header1);
			$worksheet->write(M2,iconv("utf-8","tis-620","ชม."), $xlscelldesc_header1);
			$worksheet->write(N2,iconv("utf-8","tis-620","กิจกรรม"), $xlscelldesc_header1);
			$worksheet->write(O2,iconv("utf-8","tis-620","ยุทธศาสตร์ ม."), $xlscelldesc_header1);
			$worksheet->write(P2,iconv("utf-8","tis-620","ยุทธสาสตร์ คณะฯ"), $xlscelldesc_header1);
			$worksheet->write(Q2,iconv("utf-8","tis-620","งบประมาณ"), $xlscelldesc_header1);
			$worksheet->write(R2,iconv("utf-8","tis-620","หน่วยงานผู้จัด"), $xlscelldesc_header1);
			$worksheet->write(S2,iconv("utf-8","tis-620","จำนวนผู้เข้าร่วม"), $xlscelldesc_header1);
			$worksheet->write(T2,iconv("utf-8","tis-620","ผู้บันทึกข้อมูล"), $xlscelldesc_header1); 
            $xlsRow = 3;
			...
			$sql_1= "select * from $db_eform.develop
				inner join $db_eform.develop_course_personel on (develop.dev_id = develop_course_personel.dev_id)
						...
						and develop.act_id = '$_GET[act_id]'
            ...
            # ตรงนี้คือดึงข้อมูลจาก mysql มาใส่ใน Cell
				...
				//งบประมาณ
                        ...
                        $worksheet->write("A$xlsRow", "$i", $xlsCellDesc);
						 $worksheet->write("B$xlsRow", '', $xlsCellDesc);
						 $worksheet->write("C$xlsRow", $ob['dev_id'], $xlsCellDesc);
						$worksheet->write("D$xlsRow", iconv("utf-8","tis-620",$ob['dp_name']),  $xlsCellDesc);
						$worksheet->write("E$xlsRow", iconv("utf-8","tis-620",$ob['dp_name']),  $xlsCellDesc);
						$worksheet->write("F$xlsRow", iconv("utf-8","tis-620",$ob['dev_onus']),  $xlsCellDesc);
						$worksheet->write("G$xlsRow", iconv("utf-8","tis-620",$ob['dev_place'].' ('.$ob['lt_title'].', '.$cf_local[$ob['dev_local']]['name'].')'),  $xlsCellDesc);
						$worksheet->write("H$xlsRow", iconv("utf-8","tis-620",dateFormat($ob['dev_stdate'])),  $xlsCellDesc);
						$worksheet->write("I$xlsRow", iconv("utf-8","tis-620",dateFormat($ob['dev_enddate'])),  $xlsCellDesc);
						$worksheet->write("J$xlsRow", iconv("utf-8","tis-620",$devMaintype01),  $xlsCellDesc_02);
						$worksheet->write("K$xlsRow", iconv("utf-8","tis-620",$devMaintype02),  $xlsCellDesc_02);
						$worksheet->write("L$xlsRow", iconv('utf-8','tis-620',substr($dvt_name,'0','-2')),  $xlsCellDesc);
						$worksheet->write("M$xlsRow", iconv("utf-8","tis-620",$ob['dev_training_hour']),  $xlsCellDesc);
						$worksheet->write("N$xlsRow", iconv("utf-8","tis-620",$ob['activity']),  $xlsCellDesc);
						$worksheet->write("O$xlsRow", iconv("utf-8","tis-620",$ob['nameth']),  $xlsCellDesc);
						$worksheet->write("P$xlsRow", iconv("utf-8","tis-620",$ob['sf_name']),  $xlsCellDesc);
						$worksheet->write("Q$xlsRow", iconv('utf-8','tis-620',$budget),  $xlsCellDesc);
						$worksheet->write("R$xlsRow", iconv("utf-8","tis-620",$ob['dev_org']),  $xlsCellDesc);
						$worksheet->write("S$xlsRow", iconv("utf-8","tis-620",$ob['per_pname'].' '.$ob['per_fnamet'].' '.$ob['per_lnamet']), $xlsCellDesc);
						$worksheet->write("T$xlsRow", iconv("utf-8","tis-620",$ob['per_pname'].' '.$ob['per_fnamet'].' '.$ob['per_lnamet']), $xlsCellDesc);
                $xlsRow++;
                }				
				//จำนวนรวม
				/*$worksheet->set_row($xlsRow, 22);
